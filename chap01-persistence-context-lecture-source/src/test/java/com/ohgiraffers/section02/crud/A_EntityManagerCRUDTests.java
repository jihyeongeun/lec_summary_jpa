...
public class A_EntityManagerCRUDTests {
    ...
    public void 새로운_메뉴_추가_테스트(){
        ...
        //then
         Assertions.assertTrue(entityManager.contains(menu));  // menu 객체가 현재 영속상태로 관리되는지?
    }
...
//        System.out.println("menu = " + menu);
        ...
        String menuNameToChage = "갈치스무디";
        //when
        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();

        try {
            menu.setMenuName(menuNameToChage);
            entityTransaction.commit();
        }catch (Exception e){
            entityTransaction.rollback();
        }
        ...
        //then
        Assertions.assertEquals(menuNameToChage, entityManager.find(Menu.class, 2).getMenuName());
    }
