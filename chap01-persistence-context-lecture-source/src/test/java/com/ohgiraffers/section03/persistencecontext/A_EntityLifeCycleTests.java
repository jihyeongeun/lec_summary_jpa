...
public class A_EntityLifeCycleTests {
    ...
    /* 필기.
     *  영속성 컨텍스트는 엔티티 매니저가 엔티티객체를 저장하는 공간으로 엔티티 객체를 보관하고 관리한다.
...
//        System.out.println("foundMenu = " + foundMenu);
    ...
    public void 준영속성_detach_테스트(){
        Menu foundMenu1 = entityManager.find(Menu.class, 11);
        Menu foundMenu2 = entityManager.find(Menu.class, 12);    // entity manager는 자기가가지고있으면 값을 주고 안가지고 있으며 select를 통해 값을 가져와서 꺼내줌

        /* 설명.
         *  영속성 컨텍스트가 관리하던 엔티티 객체를 관리하지 않는 상태가 되게 한 것을 준영속 상태라고 한다.
         *  detach가 준영속 상태를 만들기 위한 메소드이다.
        * */
        entityManager.detach(foundMenu2);            // 개념이해를 위함, 많이 쓸 일 없음.
...
//        entityManager.clear();
        ...
        /* 설명. 영속성 컨텍스트 및 엔티티매니저까지 종료해 버린다.(사용불가) */
        entityManager.close();
